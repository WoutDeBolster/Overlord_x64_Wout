...
void SoftwareSkinningScene_2::Update()
{
	...
	for (size_t i = 0; i < m_SkinnedVertices.size(); i++)
	{
		if (i <= 23)
		{
			XMVECTOR transfromedVertex{ XMVector3TransformCoord(XMLoadFloat3(&m_SkinnedVertices[i].originalVertex.Position), boneTransfrom0) };
			XMStoreFloat3(&m_SkinnedVertices[i].transformedVertex.Position, transfromedVertex);
		}
		else
		{
			XMVECTOR transfromedVertex{ XMVector3TransformCoord(XMLoadFloat3(&m_SkinnedVertices[i].originalVertex.Position), boneTransfrom1) };
			XMStoreFloat3(&m_SkinnedVertices[i].transformedVertex.Position, transfromedVertex);
		}
	}
