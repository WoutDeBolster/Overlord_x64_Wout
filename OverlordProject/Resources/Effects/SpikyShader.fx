...
// GEOMETRY SHADER *
//******************
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float3 normal, float2 texCoord) // textcoord nodig??
{
...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
	...
	//Step 4 + 5. Calculate Individual Face Normals (Cross Product of Face Edges) & Create Vertices for every face
        ...
        //FACE 1
        float3 faceNormal1 = normalize(cross(basePoint - vertices[1].Position, basePoint - vertices[0].Position));
		CreateVertex(triStream, vertices[0].Position, faceNormal1, (float2)basePoint);
        CreateVertex(triStream, vertices[1].Position, faceNormal1, (float2)basePoint);
        CreateVertex(triStream, basePoint, faceNormal1, (float2)basePoint);

        //Restart Strip! >> We want to start a new triangle (= interrupt trianglestrip)
        triStream.RestartStrip();

        //FACE 2
        float3 faceNormal2 = normalize(cross(basePoint - vertices[2].Position, basePoint - vertices[1].Position));
		CreateVertex(triStream, vertices[1].Position, faceNormal2, (float2)basePoint);
        CreateVertex(triStream, vertices[2].Position, faceNormal2, (float2)basePoint);
        CreateVertex(triStream, basePoint, faceNormal2, (float2)basePoint);
		
		triStream.RestartStrip();
      
        //FACE 3
        float3 faceNormal3 = normalize(cross(basePoint - vertices[0].Position, basePoint - vertices[2].Position));
		CreateVertex(triStream, vertices[2].Position, faceNormal3, (float2)basePoint);
        CreateVertex(triStream, vertices[0].Position, faceNormal3, (float2)basePoint);
        CreateVertex(triStream, basePoint, faceNormal3, (float2)basePoint);
