...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord, float ks)
{
	float3 halfVector = -normalize(viewDirection + gLightDirection);
	float cosA = saturate(dot(normal, halfVector));
	float blinSpec = ks * pow(cosA, gShininess);
	
	return blinSpec;
}

float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord, float ks)
{
	float3 phongReflect = reflect(-gLightDirection, normal);
	float cosA = saturate(dot(phongReflect, viewDirection));
	float phongSpecReflec = ks * pow(cosA, gShininess);
...
float3 CalculateSpecular(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	{
		ks = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord).r;
	}
	
	float specBlin = 0;
	if(gUseSpecularBlinn)
	{
		specBlin = CalculateSpecularBlinn(viewDirection, normal, texCoord, ks);
	}
	
	float specPhong = 0;
	if(gUseSpecularPhong)
	{
		specPhong = CalculateSpecularPhong(viewDirection, normal, texCoord, ks);
	}
	
	float reflactant = specBlin + specPhong;
	
	return reflactant * gColorSpecular;
}
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	if(gUseTextureNormal)
	{
		...
		newNormal = gTextureNormal.Sample(gTextureSampler, texCoord).rgb;
		
		newNormal.r = 2.0 * newNormal.r - 1.0;
        newNormal.r /= 255.0;
        
        newNormal.g = 2.0 * newNormal.g - 1.0;
        newNormal.g /= 255.0;
        
        newNormal.b = 2.0 * newNormal.b - 1.0;
        newNormal.b /= 255.0;
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	float fresnelResult = fresnel * fresnelMask;
	fresnelFalloff = fresnelResult;
	
	if(gUseEnvironmentMapping)
	{
		return gColorFresnel * fresnelFalloff * environmentColor;
	}
	
	return fresnelFalloff * gColorFresnel;
}

float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	float3 enviormentColor = float3(0,0,0);
	
	if(gUseEnvironmentMapping)
	{
		float3 reflectVector = reflect(viewDirection, normal) * gReflectionStrength;
		float3 refractColor = refract(viewDirection, normal, gRefractionIndex) * gRefractionStrength;
		
		enviormentColor = gCubeEnvironment.Sample(gTextureSampler, reflectVector);
		enviormentColor += refractColor.r;
	}
	
	return enviormentColor;
}
